  ...
  PIRP Irp)
{
    ...
    * style calls. */
        ...
        TDI_TRANSPORT_ADDRESS_LENGTH) == TDI_TRANSPORT_ADDRESS_LENGTH))
    {
        ...
        Status = FileOpenAddress(&Request, Address, (USHORT)Protocol, Shared, NULL);
        if (NT_SUCCESS(Status))
        {
 ...
 */
{
  ...
  DbgPrint("TiDispatchOpenClose Called. DeviceObject is at (0x%X), IRP is at (0x%X).\n", DeviceObject, Irp);
 ...
 */
{
  ...
  IrpSp = IoGetCurrentIrpStackLocation(Irp);
  DbgPrint("TiDispatchInternal Called. DeviceObject is at (0x%X), IRP is at (0x%X).\n", DeviceObject, Irp);
